...
function rpress_user_info_fields() {
	...
	<fieldset id="rpress_checkout_user_info">
		...
		<p id="rpress-phone-wrap" class="rp-col-md-6 rp-col-sm-12">
			<label class="rpress-label" for="rpress-phone">
			<?php esc_html_e('Phone Number', 'restropress'); ?>
			<?php if( rpress_field_is_required( 'rpress_phone' ) ) { ?>
				<span class="rpress-required-indicator">*</span>
			<?php } ?>
			</label>
      		<input class="rpress-input required" type="text" name="rpress_phone" id="rpress-phone" value="<?php echo esc_attr( $customer['phone'] ); ?>" placeholder="<?php esc_html_e('Phone Number', 'restropress'); ?>" maxlength="16" <?php if( rpress_field_is_required( 'rpress_phone' ) ) {  echo 'required'; } ?> />
    	</p>
...
<fieldset id="rpress_checkout_order_details">
	...
	<?php
		if( rpress_selected_service() == 'delivery' ) :
			...
			// Custom code
			$arrPostCodes = RPRESS()->fees->getArrPostCodes();
	...
	?>
		...
		<p id="rpress-postcode" class="rp-col-md-6 rp-col-sm-12">
			...
			<select class="rp-form-control rpress-postcode" id="rpress-postcode" name="rpress_postcode">
				<option hidden selected></option>
				<?php
				if ( is_array( $arrPostCodes ) ) :
				foreach( $arrPostCodes as $k => $v) :
				?>
...
add_action( 'rpress_after_payment_gateways', 'rpress_discount_field', -1 );
