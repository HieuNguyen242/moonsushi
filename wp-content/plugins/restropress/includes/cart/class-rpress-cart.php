...
class RPRESS_Cart {
	...
	public function get_contents_details() {
		...
		foreach ( $this->contents as $key => $item ) {
			...
			$total = $subtotal - round($discount, rpress_currency_decimal_filter()) + $tax;
	...
	public function get_item_discount_amount( $item = array(), $discount = false ) {
		...
		$price = $this->get_item_price( $item['id'], $item['options'], $price_id, false );
		...
		if ( ! empty( $discounts ) ) {
						...
						if( 'flat' === rpress_get_discount_type( $code_id ) ) {
							...
							foreach ( $cart_items as $cart_item ) {
								if ( ! in_array( $cart_item['id'], $excluded_products ) ) {
									...
									$item_price = $this->get_item_price( $cart_item['id'], $options, $price_id, false );
			...
			$amount = round( ( $price - apply_filters( 'rpress_get_cart_item_discounted_amount', $discounted_price, $discounts, $item, $price ) ), 6 );
	...
	public function get_total( $discounts = false ) {
		...
		$total_wo_tax = $subtotal - round($discounts, rpress_currency_decimal_filter()) + $fees;
		$total        = $subtotal - round($discounts, rpress_currency_decimal_filter()) + $cart_tax + $fees ;
		...
		return round( $this->total, rpress_currency_decimal_filter() );
	}
