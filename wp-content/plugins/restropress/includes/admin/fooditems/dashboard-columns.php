...
function add_addons_status_columns( $columns ) {
    $columns['addon-status'] = 'Status';
    return $columns;
}
add_filter( 'manage_edit-addon_category_columns', 'add_addons_status_columns' );

function add_addons_status_column_content( $content, $column_name, $term_id ) {
    $term = get_term( $term_id, 'addon_category' );
	$term_meta = get_option( "taxonomy_term_$term->term_id" );
	$addon_statuses = rpress_get_addon_statuses();
	$current_addon_status = isset($term_meta['status']) ? $term_meta['status'] : 'active';
	$options = '<div class="rp-addon-status-wrapper"><select data-addon-id="'.$term_id.'" class="rp-addon-status-update-on-change rp-addon-status-' . $current_addon_status . '">';
	...
	$options .= '</select>';
	$options .= '<span class="order-status-loading"></span></div>';
	return $options ;
}
add_filter( 'manage_addon_category_custom_column', 'add_addons_status_column_content', 9, 3 );