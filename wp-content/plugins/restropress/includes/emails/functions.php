...
function rpress_admin_email_notice( $payment_id = 0, $payment_data = array() ) {
	...
	try {
		$enableMobileAppNotification = defined( 'Enable_Mobile_App_Notification' ) ? Enable_Mobile_App_Notification : false;
		if (!$enableMobileAppNotification) {
			return;
		}
		$googleFCMAuthorization = defined( 'Google_FCM_Authorization' ) ? Google_FCM_Authorization : '';
		$srvAPIKey = defined( 'API_Key' ) ? API_Key : '';

		$response = wp_remote_post( 'https://fcm.googleapis.com/fcm/send', array(
			'method'      => 'POST',
			'headers'     => array(
				'Authorization' => $googleFCMAuthorization,
				'Content-Type' => 'application/json'
			),
			'body'        => wp_json_encode( array(
				'to' => ('/topics/new_order_' . $srvAPIKey),
				'data' => array(
					'message' => $subject,
					'orderId' => $payment_id
				),
