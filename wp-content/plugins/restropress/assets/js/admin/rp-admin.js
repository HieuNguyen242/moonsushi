...
/**
* Addon status change by dropdown
*/
jQuery( document ).on( 'change', 'select.rp-addon-status-update-on-change', function(e) {
  e.preventDefault();
  var _self           = jQuery( this );
  var selectedStatus  = _self.val();
  var term_id      = _self.attr( 'data-addon-id' );

  if ( selectedStatus !== '' ) {
    _self.parent( '.rp-addon-status-wrapper' ).find( '.order-status-loading' ).addClass( 'disabled' );
    ...
    jQuery.ajax({
      ...
      data:    {
        ...
        action  : 'rpress_update_addon_status',
      },
      type:    'GET',
      success: function( response ) {
        _self.parent( '.rp-addon-status-wrapper' ).find( '.order-status-loading' ).removeClass( 'disabled' );
        _self.removeClass( 'rp-addon-status-active' );
        _self.removeClass( 'rp-addon-status-inactive' );
        _self.addClass( 'rp-addon-status-' + selectedStatus);
      }
    }).fail( function ( data ) {
      if ( window.alert ) {
        window.alert( data );
