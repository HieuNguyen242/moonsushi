window.RPRESS_Checkout = (function($) {
	...
	function init() {
		...
		$body.on('rpress_gateway_loaded', function( e ) {
			...
			$('#rpress-discount-code-wrap').show();
		});
		...
		$checkout_form_wrap.on('change', '.rpress-postcode', apply_fee);
	...
	function apply_fee(event) {

		event.preventDefault();

		var $this = $(this),
			postcode = $('select#rpress-postcode').val();
				...
				if( response ) {
					if (response.msg == 'valid') {
						$('.rpress_cart_amount').each(function() {
							...
							$( this ).data( 'total', response.total_plain );
							if ($('#rpress_cart_fee_lieferkosten').length == 0) {
								$('<tr class="rpress_cart_fee" id="rpress_cart_fee_lieferkosten"></tr>').insertBefore('.rpress_cart_footer_row');
							}
							$('#rpress_cart_fee_lieferkosten').html(response.html);
						});
					}
				} else {
